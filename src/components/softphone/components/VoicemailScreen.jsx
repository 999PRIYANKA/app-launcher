    ...
    <div className="h-full bg-white">
      <ul>
        {mockVoicemails.map((vm) => (
          <li key={vm.id} className="px-8 py-5 border-b border-gray-100 hover:bg-gray-50 transition-colors">
            <button onClick={() => onCall(vm.number)} className="w-full text-left group">
                <div className="flex justify-between items-center mb-2">
                    <p className="text-2xl text-slate-900 font-semibold group-hover:text-blue-600 transition-colors">{vm.name || vm.number}</p>
                    <span className="text-lg text-gray-400">{vm.date}</span>
                </div>
                <div className="flex justify-between items-center">
                    <p className="text-lg text-gray-500">{vm.name ? vm.number : 'Unknown'}</p>
