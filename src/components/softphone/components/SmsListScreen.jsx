                ...
                >
                    ...
                    <div className="flex-grow min-w-0 pt-1">
                        <div className="flex justify-between items-baseline mb-2">
                            <h3 className="text-1xl font-bold text-slate-900 truncate">{thread.name || thread.number}</h3>
                            <span className="text-base text-gray-400 flex-shrink-0 ml-3">
                                {lastMessage ? lastMessage.timestamp : ''}
                            </span>
                        </div>
                        <div className="flex justify-between items-center">
                            <p className={`text-sl truncate pr-3 ${thread.unread ? 'font-semibold text-slate-900' : 'text-gray-500'}`}>
                                {lastMessage ? (
