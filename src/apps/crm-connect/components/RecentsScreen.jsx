    ...
    <div className="h-full bg-white">
      <ul>
        {mockRecents.map((call) => (
          <li key={call.id} className="flex items-center justify-between px-7 py-6 border-b border-gray-100 hover:bg-gray-50 transition-colors group">
            <button onClick={() => onCall(call.number)} className="flex-grow text-left flex items-center space-x-5">
              <div className="flex-grow">
                <p className={`text-3xl font-medium ${call.type === 'missed' ? 'text-red-500' : 'text-slate-900'}`}>
