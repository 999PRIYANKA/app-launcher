...
const ActionButton = ({ label, icon, isActive = false, onClick }) => (
  <div className="flex flex-col items-center space-y-4">
    <button
      onClick={onClick}
      className={`rounded-full w-28 h-28 flex items-center justify-center transition-colors focus:outline-none ${
...
const InCallScreen = ({ number, callState, onEndCall }) => {
  ...
  return (
    <div className="bg-zinc-900 text-white h-full flex flex-col justify-between p-10">
      <div className="h-1/4 flex flex-col justify-end items-center space-y-4">
        <p className="text-6xl font-light text-center leading-tight">{number}</p>
        <p className="text-gray-400 text-2xl">{getStatusText()}</p>
      </div>
      ...
      <div className="h-1/4 flex items-center justify-center">
        {isCallActive && (
          <button
            onClick={onEndCall}
            className="bg-red-500 text-white rounded-full w-28 h-28 flex items-center justify-center transition-colors focus:outline-none hover:bg-red-600"
          >
