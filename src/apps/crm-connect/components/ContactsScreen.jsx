            ...
            <ul>
              {groupedContacts[letter].map((contact) => (
                <li key={contact.id} className="border-b border-gray-100 last:border-0">
                  <button onClick={() => onCall(contact.number)} className="w-full text-left px-6 py-5 hover:bg-gray-50 transition-colors group">
                    <div className="text-2xl font-bold text-slate-900 group-hover:text-blue-600 transition-colors mb-1">
                        {contact.name}
                    </div>
