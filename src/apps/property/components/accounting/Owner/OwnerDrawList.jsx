...
const OwnerDrawList = ({ draws, onAddClick, onSelect }) => {
  ...
  const filteredData = useMemo(() => {
    ...
    return processedData.filter(draw =>
      draw.drawNumber?.toLowerCase().includes(search) ||
      draw.ownerName?.toLowerCase().includes(search)
    );
  ...
  return (
    <div className="flex h-full bg-white flex-col p-6">
        <div className="flex justify-between mb-6">
            ...
            >
                <Icons.PlusCircleIcon className="w-4 h-4 mr-2" /> Record Draw
            </button>
        </div>
